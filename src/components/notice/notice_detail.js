...
class NoticeDetail extends Component {
    ...
    handleSubmitData(e) {
        ...
        setTimeout(() => {
            fetch(`/api/notice/${this.state.jsonData.id}`, options)
            .then(_response => _response.json())
            .then(_data => {
                if (_data.error) {
    ...
    handleRadioType(e) {
        ...
        if (e.currentTarget.value === 'noti') {
            newObj = Object.assign(this.state.jsonData, {type: 'noti'});
        } else {
            newObj = Object.assign(this.state.jsonData, {type: 'normal'});
        }
                ...
                return (
                    <form className="border border-light p-4">
                        ...
                        <div className="form-group">
                            ...
                            <label className="col-sm-2 control-label" htmlFor="article_title">공지 유형</label>  
                            <div className="col-sm-2">
                                <span className="col-sm-6">
                                    <label className="form-check-label">
                                        <input type="radio" className="form-check-input" name="notice_type" id="notice_type1" value="noti" checked={this.state.jsonData.type === 'noti' ? true : false} onChange={this.handleRadioType}/>
                                        공지
                                    </label>
                                </span>
                                <span className="col-sm-6">
                                    <label className="form-check-label">
                                        <input type="radio" className="form-check-input" name="notice_type" id="notice_type2" value="normal" checked={this.state.jsonData.type === 'normal' ? true : false} onChange={this.handleRadioType} />
                                        일반
                                    </label>
                                </span>
                            </div>
...
export default connect(null, mapDispatchToProps)(NoticeDetail);